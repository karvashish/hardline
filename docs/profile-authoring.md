# Profile Authoring Guide

This page is the profile-authoring docs hub. The detailed reference is split into smaller topic files.

## Start Here

- [Hello World Profile](profiles/hello-world.md)
- [Profile Structure](profiles/structure.md)
- [Action Files](profiles/action-files.md)
- [Built-In Plugins](profiles/builtin-plugins.md)
- [Overrides And Signing](profiles/overrides-and-signing.md)
- [External Plugins](profiles/external-plugins.md)

## Suggested Reading Order

1. [Hello World Profile](profiles/hello-world.md)
2. [Profile Structure](profiles/structure.md)
3. [Action Files](profiles/action-files.md)
4. [Built-In Plugins](profiles/builtin-plugins.md)
5. [Overrides And Signing](profiles/overrides-and-signing.md)

## Example

The main example profile lives in [`profiles/starter-secure-ubuntu-24.04-lts/profile.json`](https://github.com/karvashish/hardline/blob/main/profiles/starter-secure-ubuntu-24.04-lts/profile.json).
