...
- Hardline ensures the main config starts with `flush ruleset`, so one load produces exactly the ruleset the file describes rather than adding to whatever the kernel already holds. Debian-family configs ship the line; on RHEL-family hosts hardline writes it under a `# hardline:` marker comment. Every managed include in that file needs the header, so which run wrote it does not decide who removes it: rollback takes the header out when its own include was the last managed one left and the marker says hardline put it there, and leaves a bare `flush ruleset` the operator wrote alone
...
- before loading, the `input` chain must accept a new tcp connection on every port sshd is listening on, or apply refuses rather than locking itself out. Hardline cannot tell which listener carries this run's session, so an accepted `22` does not excuse a blocked `2222`. The chain is walked in declared order, the way the kernel evaluates it: the first unscoped rule that a new connection to that port reaches decides, and only if no rule matches does the chain policy decide. An `accept` sitting behind a `drop` for the same port never gets reached and does not count, and neither does an `accept` whose `ct_states` exclude `new`, since a new connection never matches it. A `drop` or `reject` narrowed by an address or an interface is refused as unevaluable rather than guessed at: hardline cannot tell whether it covers this run's own connection, and moving the accept above it settles the question. The guard runs whenever the policy is `drop`/`reject` or any input rule could deny new tcp traffic, so a permissive policy with one hostile rule is caught too
- the port is read from `ss` where sshd holds the listener itself, and from an active `ssh.socket`/`sshd.socket` where systemd holds it instead, which is the default on Ubuntu 24.04; a port a socket unit declares counts only while something is really listening on it. The probe fails closed: if the listening port cannot be determined, nothing is loaded
- the `output` chain is held to the same standard: an ssh reply leaves as an established packet, and the spec has no source-port vocabulary, so a `drop`/`reject` output policy needs an unscoped `accept` for the `established` state to be shown to carry it. Without one, apply refuses rather than letting the host stop answering
...
- the capture records the loaded table from `nft -j list ruleset` alongside the file and the include lines. Apply loads the kernel on every run, so a run that finds all three files correct but the kernel drifted still changes the host, and a file-only capture would show no delta for it and rollback would skip a step that did reload the ruleset
