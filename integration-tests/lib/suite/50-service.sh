...
# Uses the target's cron unit (always present, safe to bounce) as the test
# unit: Debian-family calls it cron, RHEL-family crond, so it comes from os.sh.

_cron_restore() { ssh_cmd "sudo systemctl enable --now ${CRON_UNIT}" >/dev/null 2>&1 || true; }
...
EOF
  ...
  ssh_cmd "sudo systemctl enable --now ${CRON_UNIT}" >/dev/null 2>&1 || true
  local s1 s2; s1="$(svc_stamp ${CRON_UNIT})"
  p=$(make_profile_service "svc-restarted" "${CRON_UNIT}" "restarted" "true")
  must_hl "${dir}/restarted.log" "apply restarted" -- apply "${p}" "${remote_args[@]}"
  s2="$(svc_stamp ${CRON_UNIT})"
  [ -n "${s1}" ] && [ "${s1}" != "${s2}" ] || note_fail "restarted did not move MainPID/ActiveEnterTimestamp (${s1} -> ${s2})"
  ssh_cmd "systemctl is-active ${CRON_UNIT} >/dev/null 2>&1" || note_fail "${CRON_UNIT} not active after restart"

  # reload-or-restart -> still active.
  p=$(make_profile_service "svc-ror" "${CRON_UNIT}" "reload-or-restart" "true")
  must_hl "${dir}/ror.log" "apply reload-or-restart" -- apply "${p}" "${remote_args[@]}"
  ssh_cmd "systemctl is-active ${CRON_UNIT} >/dev/null 2>&1" || note_fail "${CRON_UNIT} not active after reload-or-restart"
...
scenario_service_onchange_skip() {
  ...
  local p; p=$(make_profile_template_service "svc-onchange" "${dest}" "OnChange=yes" "${CRON_UNIT}")
  ...
  fp1="$(fp_path "${dest}")"
  mark0="$(svc_actmark ${CRON_UNIT})"; cur="$(journal_cursor)"
  ...
  if svc_acted_since ${CRON_UNIT} "${mark0}" "${cur}"; then
    note_fail "${CRON_UNIT} was reloaded/restarted on a no-op apply (should be untouched)"
  fi
...
scenario_service_reload_rollback() {
  ...
  reset_dir "${dir}"
  scenario_start "service-reload-rollback: rollback re-runs the reload of active ${CRON_UNIT} whose config it reverts (observed via systemd)"
  guard_can_sign || return

  _cron_restore
  local dest="/etc/hardline.d/99-hardline-svc-reload-rb.conf"
  ssh_cmd "sudo rm -f ${dest}"
  local p; p=$(make_profile_template_service "svc-reload-rb" "${dest}" "ReloadRollback=yes" "${CRON_UNIT}")
  ...
  local mark0 cur; mark0="$(svc_actmark ${CRON_UNIT})"; cur="$(journal_cursor)"
  must_hl "${dir}/rollback.log" "rollback" -- rollback "${p}" "${remote_args[@]}"

  svc_acted_since ${CRON_UNIT} "${mark0}" "${cur}" \
    || note_fail "${CRON_UNIT} not reloaded/restarted on rollback (independent: MainPID + StateChange unchanged, no journal reload)"
...
scenario_service_policy_always() {
  ...
  ssh_cmd "sudo rm -f ${dest}"
  local p; p=$(make_profile_template_service "svc-always" "${dest}" "AlwaysReload=yes" "${CRON_UNIT}" "always")

  must_hl "${dir}/apply.log" "apply" -- apply "${p}" "${remote_args[@]}" --keep-local-rollback || { ssh_cmd "sudo rm -f ${dest}"; scenario_end; return; }
  local mark0 cur; mark0="$(svc_actmark ${CRON_UNIT})"; cur="$(journal_cursor)"
  must_hl "${dir}/rollback.log" "rollback" -- rollback "${p}" "${remote_args[@]}"
  svc_acted_since ${CRON_UNIT} "${mark0}" "${cur}" \
    || note_fail "type=always reload not re-run on rollback (independent: ${CRON_UNIT} untouched)"
...
scenario_service_conflict() {
  ...
  ssh_cmd "sudo systemctl start ${CRON_UNIT}" >/dev/null 2>&1 || true
  local p; p=$(make_profile_service "svc-conflict" "${CRON_UNIT}" "stopped" "true")
  must_hl "${dir}/apply.log" "apply (stop ${CRON_UNIT})" -- apply "${p}" "${remote_args[@]}" --keep-local-rollback || { _cron_restore; scenario_end; return; }
  ssh_cmd "sudo systemctl start ${CRON_UNIT}" >/dev/null 2>&1 || true   # drift: current active != recorded inactive
