...
func TestEnforceAbsCleanPath(t *testing.T) {
	...
	pass := map[string]string{
		...
		"/etc/foo.bar-baz_qux+v=1@x":     "/etc/foo.bar-baz_qux+v=1@x",
	...
	fail := map[string]string{
		...
		"..":         "absolute",
		"~/x":        "absolute",
		// leading whitespace (rejected as non-printable before the absolute check)
		" /etc/shadow":  "printable ASCII",
		"\t/etc/shadow": "printable ASCII",
		// trailing / embedded whitespace
		"/etc/shadow ": "printable ASCII",
		"/etc/sh adow": "printable ASCII",
		// control characters, NUL, newline, CR, DEL
		"/etc/sh\nadow":   "printable ASCII",
		"/etc/sh\tadow":   "printable ASCII",
		"/etc/sh\radow":   "printable ASCII",
		"/etc/sh\x00adow": "printable ASCII",
		"/etc/\x7f":       "printable ASCII",
		// non-ASCII / unicode homoglyph
		"/etc/café":       "printable ASCII",
		"/etc/sh\xffadow": "printable ASCII",
		// path traversal
