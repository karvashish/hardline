...
// acknowledgement of collateral: apt resolves a purge outwards, and every extra
// package the transaction takes has to be named here before apply will run it.
type Spec struct {
...
func restore(host pluginapi.Host, p pluginapi.PackageState) error {
	...
	if p.RequestedInstall && !p.WasInstalled {
		// apt resolves a removal through reverse dependencies, so an unguarded
		// undo takes whatever came to depend on the package since apply along
		// with it. Read the transaction back first and stop rather than remove
		// more than one install's worth.
		preview, err := purgePreview(host, []string{name})
		if err != nil {
...
func conflict(host pluginapi.Host, p pluginapi.PackageState) []string {
	...
	current, version, _, err := query(host, name)
	if err != nil {
