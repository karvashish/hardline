...
func TestAdapter(t *testing.T) {
	...
	if !strings.Contains(lockCheck, "/var/cache/dnf/metadata_lock.pid") ||
		strings.Contains(lockCheck, "/var/cache/libdnf5/") {
		t.Fatalf("dnf4 lock probe is wrong:\n%s", lockCheck)
	}
	...
	if want := `LC_ALL=C dnf -q check-update; rc=$?; [ "$rc" = 100 ] && exit 0; exit $rc`; host.command != want {
		t.Fatalf("upgrade command=%q, want %q", host.command, want)
	}
	host.output = "Dependencies resolved.\n"
	if _, err := b.Previews.Install(host, []string{"tree"}); err != nil {
		t.Fatalf("dnf4 transaction proof was rejected: %v", err)
	}

	p := Plugin()
	if p.Name != "packages_dnf4" || !p.InternalValidation {
