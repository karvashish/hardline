...
const checkUpdateOutput = `Last metadata expiration check: 0:00:01 ago on Mon 01 Jan 2026.
...
grub2-tools.x86_64             1:2.06-80.el9          baseos
    grub2-tools-efi.x86_64     1:2.06-70.el9          @baseos
`
...
func step(t *testing.T, config map[string]any) profile.Step {
	t.Helper()
	return profile.Step{ID: "pkg", Plugin: "packages_dnf4", Config: config}
}

func TestPluginIdentity(t *testing.T) {
	p := Plugin()
	if p.Name != "packages_dnf4" {
...
func TestParseCheckUpdateCountsObsoletingReplacements(t *testing.T) {
	got := parseCheckUpdate(checkUpdateOutput)
	...
	// The indented row is the package being replaced, not a package the
	// transaction installs.
	for _, name := range got {
		if name == "grub2-tools-efi" {
			t.Error("the obsoleted package must not be counted as a change")
		}
	}
}
...
func TestPreviewsTranslateOnlyTheirOwnExitCodes(t *testing.T) {
	if !strings.Contains(checkUpdateCmd, `[ "$rc" = 100 ]`) {
		t.Error("check-update must translate exit 100 only")
	}
...
func TestPlan(t *testing.T) {
	host := hostStub{
		...
		output: map[string]string{
			"check-update":   checkUpdateOutput,
